...
if searchflag "${uarg[@]}"; then
  ...
  for usr in ${!pidmap[@]}; do
    printf "  %-10s: \e[36m%s\e[0m\n" "$usr" "${pidmap[$usr]}"
  done && echo
else
  for arg in "${uarg[@]}"; do
    if [[ -z $(getent passwd | grep -e "^$arg:") ]]; then echo -e "\e[33m User $arg does not exist on this system.\e[0m" && continue; fi
    psu=`ps -fu $arg | grep "\-bash$" | awk '{print $2}'`
    if [[ -z $psu ]]; then echo -e "\e[33m User $arg does not have active login shells.\e[0m" && continue; fi
    pids+=($psu)
    echo -e "$arg - $((${#pids[@]})) active login shell(s): \e[31m"$psu"\e[0m"
  done
fi
[[ -z $pids ]] && { echo -e "\e[33m PID list empty, terminating. \e[0m"; exit; }
...
for pid in "${pids[@]}"; do
  if [[ $pid == $cpid ]]; then
    ...
    if [[ -z $error ]]; then echo -e "\e[36mdone\e[0m"; else echo -e "\e[31m$error\e[0m"; fi
  fi
done
